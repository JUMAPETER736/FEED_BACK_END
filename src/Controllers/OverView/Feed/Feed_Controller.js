...
const getFollowedPosts = asyncHandler(async (req, res) => {
  ...
  console.log("1");
  const postAggregation = FeedFollowUnfollow.aggregate([
    {
      $match: {
    ...
    },
    {
      $lookup: {
        ...
        foreignField: "_id",
        as: "post",
        pipeline: feedCommonAggregation(req), // after lookup we need to structure the posts same as other post apis
      },
    },
    {
      $addFields: {
  ...
  console.log("2");
  try {
    const posts = await FeedFollowUnfollow.aggregatePaginate(
      postAggregation,
      getMongoosePaginationOptions({
        page: req.query.page || 1,
        limit: req.query.limit || 10,
        customLabels: {
          totalDocs: "totalFollowedPosts",
          docs: "followedPosts",
        },
      })
    );
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, posts, "Followed posts fetched successfully"));
  } catch (error) {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          {},
          `Error fetching followed posts: ${error.message}`
        )
      );
  }
});
